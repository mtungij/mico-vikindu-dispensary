...
class PatientPersonalForm extends Form
{
    ...
    public bool $date_of_birth_is_estimated = false;
    ...
    public ?string $known_allergies = null;

    public ?string $chronic_conditions = null;

    public string $patient_status = 'active';

    public bool $profile_incomplete = false;

    public function rules(): array
    {
        $validPhone = static function (string $attribute, mixed $value, \Closure $fail): void {
            if (filled($value) && ! app(PhoneNumberService::class)->isValid((string) $value)) {
                $fail('Namba ya simu si sahihi. Tumia mfano 0712345678 au +255712345678.');
            }
        };
        ...
        return [
            ...
            'marital_status' => ['nullable', Rule::enum(MaritalStatus::class)], 'nationality' => ['required', 'string', 'max:80'], 'nida_number' => ['nullable', 'string', 'max:40'], 'passport_number' => ['nullable', 'string', 'max:40'], 'primary_phone' => ['nullable', 'string', 'max:30', $validPhone], 'secondary_phone' => ['nullable', 'string', 'max:30', $validPhone], 'email' => ['nullable', 'email'], 'region' => ['nullable', 'string', 'max:80'], 'district' => ['nullable', 'string', 'max:80'], 'ward' => ['nullable', 'string', 'max:80'], 'street_or_village' => ['nullable', 'string', 'max:100'], 'physical_address' => ['nullable', 'string', 'max:1000'], 'occupation' => ['nullable', 'string', 'max:100'], 'blood_group' => ['nullable', 'string', 'max:10'], 'known_allergies' => ['nullable', 'string'], 'chronic_conditions' => ['nullable', 'string'], 'patient_status' => ['required', 'string'], 'profile_incomplete' => ['boolean'],
        ];
    }
    ...
    public function resetForm(): void
    {
        ...
        $this->blood_group = 'unknown';
        $this->patient_status = 'active';
    }
}
