        ...
        <x-card>
            <form wire:submit="saveReceptionBilling" class="space-y-5">
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <label class="flex items-center gap-3 rounded-md border border-slate-200 p-3 text-sm dark:border-slate-700"><x-checkbox wire:model.live="charge_returning_patient_consultation" /> Enable returning-patient consultation charge</label>
                    <label class="flex items-center gap-3 rounded-md border border-slate-200 p-3 text-sm dark:border-slate-700"><x-checkbox wire:model.live="charge_emergency_consultation" /> Enable emergency consultation charge</label>
                </div>
                <div class="grid gap-4 md:grid-cols-3">
                    ...
                    <div><x-input-label value="Returning consultation service" /><x-select-input wire:model="returning_patient_consultation_service_id" class="mt-1" :disabled="!$charge_returning_patient_consultation"><option value="">Chagua</option>@foreach($consultationServices as $service)<option value="{{ $service->id }}">{{ $service->name }}</option>@endforeach</x-select-input><x-input-error :messages="$errors->get('returning_patient_consultation_service_id')" /></div>
                    <div><x-input-label value="Emergency consultation service" /><x-select-input wire:model="emergency_consultation_service_id" class="mt-1" :disabled="!$charge_emergency_consultation"><option value="">Chagua</option>@foreach($consultationServices as $service)<option value="{{ $service->id }}">{{ $service->name }}</option>@endforeach</x-select-input><x-input-error :messages="$errors->get('emergency_consultation_service_id')" /></div>
                </div>
