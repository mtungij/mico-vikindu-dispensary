...
{
    public bool $dental_require_payment_before_consultation=true; public bool $dental_require_payment_before_procedure=true; public bool $dental_bill_materials_separately=false; public bool $dental_require_consent_for_surgery=true; public bool $dental_require_signature_for_report=false; public string $dental_default_numbering_system='fdi'; public bool $dental_enable_periodontal_charting=true; public bool $dental_enable_mixed_dentition=true;
    public function mount(): void { Gate::authorize('settings.update'); foreach (array_keys($this->settings()) as $key) { $value=FacilitySetting::query()->where('facility_id',currentFacility()?->id)->where('key',$key)->value('value'); if ($value !== null) $this->{$key}=is_bool($this->{$key}) ? filter_var($value,FILTER_VALIDATE_BOOLEAN) : $value; } }
